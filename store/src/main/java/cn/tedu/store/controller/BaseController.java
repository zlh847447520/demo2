...
import cn.tedu.store.controller.ex.AccessDenideException;
import cn.tedu.store.controller.ex.AddressNotFoundException;
...
public abstract class BaseController {
	...
	/**
	 * 从Session中获取当前登录的用户的名字
	 * @param sessionsession HttpSession对象
	 * @return登录的用户的id
	 */
	...
	public JsonResult<Void> handleException(Throwable e){
		...
		if(e instanceof UsernameDuplicateException) {
			...
			jr.setState(4001);
		}else if(e instanceof PasswordNotMatcheException) {
			jr.setState(4002);
		}else if(e instanceof AddressCountLimitException) {
			jr.setState(4003);
		}else if(e instanceof AccessDenideException) {
			jr.setState(4004);
